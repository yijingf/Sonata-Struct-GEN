...
Description: 
    Normalizes note event files for both melody and accompaniment. The procedures include:
        1. Quantize irregular rhythms (e.g., 1/5, 1/7, 1/11 of a quarter note) to the nearest 1/8 or 1/12 quarter note grid.
        2. Merge small subdivisions:
            - 1/24 → 1/12
            - 1/16, 1/32 → 1/8
        3. Trim excessively long durations (greater than 8 quarter notes) to a maximum of 8 quarter notes.
        4. Normalize low-frequency duration values
        5. Normalize tempo to classical tempo markings (defined in TEMPO_BIN) and transpose pitch to C major or C minor.
...
from utils.common import get_file_list, token2v, load_note_event, normalize_event, normalize_tp
...
def quantize_irregular_rhythm(orig_dir, norm_dir):
    # Quantize irregular rhythm 1/5, 1/7, 1/11
    for event_file in sorted(get_file_list(orig_dir)):
        ...
        output_file = os.path.join(norm_dir, composer, f"{basename}.json")
        with open(output_file, "w") as f:
            json.dump(event, f)
    return


def quantize_small_div(event_file_list, is_acc=False):
    for event_file in event_file_list:
        ...
        if not is_acc or not has_chord:
...
def trim_duration(event_file_list):
...
def norm_vocab(event_file_list):
    vocab_cnt = Counter()
...
def norm_pitch(event_file_list):
    for event_file in event_file_list:
        event = load_note_event(event_file)
...
def main():
    orig_melody_dir = os.path.join(DATA_DIR, "event_part", "melody")
    melody_dir = os.path.join(DATA_DIR, "event_part", "norm_melody")

    orig_acc_dir = os.path.join(DATA_DIR, "event_part", "acc")
    acc_dir = os.path.join(DATA_DIR, "event_part", "norm_acc")

    # 1. Quantize irregular rhythm 1/5, 1/7, 1/11
    quantize_irregular_rhythm(orig_melody_dir, melody_dir)
    quantize_irregular_rhythm(orig_acc_dir, acc_dir)

    # 2. Quantize small division: 1/24 -> 1/12; 1/16, 1/32 -> 1/8
    melody_event_files = get_file_list(melody_dir)
    acc_event_files = get_file_list(acc_dir)
    quantize_small_div(melody_event_files)
    quantize_small_div(acc_event_files, is_acc=True)

    # 3. Trim extra long duration (> 8 beat) to 8 beat
    trim_duration(melody_event_files + acc_event_files)

    # 4. Normalize low frequency duration
    norm_vocab(melody_event_files + acc_event_files)

    # 5. Normalize tempo and Transpose pitch to C major/minor
    norm_pitch(melody_event_files + acc_event_files)
    return
